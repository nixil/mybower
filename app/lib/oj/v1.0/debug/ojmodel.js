...
oj.Collection.prototype._newModel = function(m, parse, options) {
    ...
    if (opt['validate'] && newModel['validate']) {
        ...
        if (validationValue) {
            ...
            this.TriggerInternal(false, oj.Events.EventType['INVALID'], this, validationValue, opt);
            return null;
        }
    }
    return newModel;
...
oj.Collection.prototype._addInternal = function(m, options, fillIn, deferred) {
        ...
        if (newModel != null) {
            ...
            if (deferred) {
                if (force) {
                    ...
                    return oj.Collection._getPromise().resolve();
                }
    ...
    if (!fillIn && (this._isVirtual() || deferred)) {
        ...
        var doTask = function(index) {
                        var defer = oj.Collection._getPromise();
        ...
        var defReturn = oj.Collection._getPromise();
        $.when(currentStep).done(function() {
            defReturn.resolve(oj.Collection._returnModels(modelReturnList));
        });
...
oj.Collection.prototype.refresh = function(options)
{
    ...
    var dfd = oj.Collection._getPromise();
    if (!this._isVirtual()) {
        ...
        this.fetch({'success': function (collection, response, options) {
                        ...
                        dfd.resolve(collection, response, options);                                    
                    },
                    'error': function (xhr, status, error) {
                        dfd.reject(self, xhr, status, error);
                    }
                    });
        return dfd;        
    }
...
oj.Collection.prototype._deferredAt = function(index, options) {
    ...
    if (model === undefined) {
        ...
        this._fetchInternal(opts, -1, false).then(resp, function(e) { dfd.reject(e);});
        return dfd;
    }
    return oj.Collection._getPromise().resolve(model);               
};
...
oj.Collection.prototype.get = function(id, options)
{
    ...
    if (internalGet) {
        ...
        if (this._isVirtual()) {
            return oj.Collection._getPromise().resolve(internalGet['m']);
        }
...
//    for (i = 0; i < this._getModelsLength(); i=i+1)
    ...
    this._getModels().some(function(model, i) {
        if (model !== undefined && model.Match(id, cid)) {
            ...
            if (deferred) {
                foundObj = oj.Collection._getPromise().resolve(retObj);
                return true;
            }
            foundObj = retObj;
            return true;
    ...
    if (this._isVirtual()) {
        ...
        this._fetchInternal(opts, -1, fillIn).then(resp, function(e) { dfd.reject(e);});
        return dfd;        
    }
    
    var undefinedModInfo = oj.Collection._getModinfo(-1, undefined);
    if (deferred) {
        return oj.Collection._getPromise().resolve(undefinedModInfo);
    }
    return undefinedModInfo;
};

oj.Collection._getPromise = function() {
    return $.Deferred();
};
 ...
 * @return {Object} a promise Object that calls done upon completion.  The promise will be passed a start and count argument that represent
...
oj.Collection.prototype.setRangeLocal = function(start, count) {
    ...
    if (actual.start === start && actual.count === count) {
        return oj.Collection._getPromise().resolve(actual.start, actual.count);
    }
    ...
    var deferredObj = oj.Collection._getPromise();
    return this._setRangeLocalInternal(start, count, -1, {start:start, count:count}, deferredObj, true);
}
  
oj.Collection.prototype._setRangeLocalInternal = function(start, count, placement, original, deferredObj, fill) {
    var self = this;
    var resp = function () {
                    ...
                    if (fill && self._hasTotalResults() && actual.count < original.count) {
                        ...
                        if (newStart < self['totalResults']) {
                            self._setRangeLocalInternal(newStart, count, newPlacement, original, deferredObj, fill);
                        }
                        else {
                            // Can't go any further
                            deferredObj.resolve(actual.start, actual.count);
                        }
                    }
                    else {
                        deferredObj.resolve(actual.start, actual.count);
    ...
    this._fetchInternal(opts, placement, placement > -1).then(resp, function(e) { deferredObj.reject(e);});
    return deferredObj;
};
...
oj.Collection.prototype.where = function(attrs, options) {
    ...
    if (deferred) {
        return oj.Collection._getPromise().resolve(arr);
    }
    return arr;
};
    ...
    if (arguments) {
        ...
        this._getModels().forEach(function(model) {
            ...
            for (j = 0; j < arguments.length; j=j+1) {
                ...
                if (model.Match(arguments[j].GetId(), arguments[j].GetCid())) {
                    add = false;
                    break;
                }
            }
...
oj.Collection.prototype.set = function(models, options) {
    var deferred = this._getDeferred(options);
    return this._setInternal(models, true, options, deferred || this._isVirtual());
};
...
oj.Collection.prototype._deferredSet = function(modelList, options, remove, add, merge, parse) {
    ...
    var doTask = function(index) {
                    var defer = oj.Collection._getPromise();
                    self._updateModel(self._newModel(modelList[index], parse, options), add, merge, true).done(function (currModel) {
                                        if (currModel !== -1) {
                                            foundModels.push(currModel);
                                        }
                                        defer.resolve(index);                                        
