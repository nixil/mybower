...
DvtDataGrid.prototype._updateGridlines = function() //vvc
{
    ...
    for (i = 1; i < rowCount; i += 1)
    {
        ...
        for (j=0; j < row.length-1; j+=1)
        {
            ...
            if (horizontalGridlines === 'hidden' || i == rowCount-1)
            {
...
DvtDataGrid.prototype._isLastRow = function(index)
{
    if (this._isCountUnknown("row"))
    {
        ...
        return (index + 1 === this.m_endRow && this.m_stopRowFetch);
    }
    else
    {
...
DvtDataGrid.prototype._isLastColumn = function(index)
{
    if (this._isCountUnknown("column"))
    {
        ...
        return (index + 1 === this.m_endCol && this.m_stopColumnFetch);
    }
    else
    {
        // if column count is known, then just check the index with the total column count
...
{
    var status, accSummary, accInfo, stateInfo, rtl, colHeader, rowHeader, scroller, databody, empty, emptyText, i;
...
DvtDataGrid.prototype.resizeGrid = function()
{
        var width, height, colHeader, rowHeader, scroller, databody,
            ...
            dir, empty, emptyText, endTime, borderWidth,scrollerHeight, scrollerWidth;
...
DvtDataGrid.prototype._adjustColumnHeader = function()
{
    ...
    if (this.m_colHeader != null && this.m_endColHeader > 0)
    {
        ...
        if ((this.getRowHeaderWidth()+this.m_endColHeaderPixel >= this.getWidth()-2))
        {
...
{
    var scrollbarSize, colHeaderHeight, rowHeaderWidth,
...
{                
    ...
    var columnScrollPosition, rowScrollPosition, indexes, initialScrollLeft, initialScrollTop = 0;    
...
DvtDataGrid.prototype.populateAccInfo = function()
{
    ...
    summary = this.getResources().getTranslatedText('accessibleSummaryExact', {'rownum': this.m_endRow, 'colnum': this.m_endCol});
...
DvtDataGrid.prototype.SetAccessibleContext = function(context)
{
    ...
    {
        this.m_stateInfo.innerHTML = context['message'];
    }
};
...
DvtDataGrid.prototype.buildColumnHeaders = function(headerRoot, headerSet, start, totalCount)
{
    var defaultHeight, scroller, renderer, totalColWidth, currentLeft, headerCount, firstHeader, headerClass, 
            dir, i, index, headerData, headerMetadata, headerContext, inlineStyle, styleClass, col, width, avgWidth, sortIcon, content, sortableAttribute, resizableAttribute, contentDiv;
    ...
    if (!headerRoot.hasChildNodes())
    {
        ...
        scroller['className'] = this.getMappedStyle("row") + " " +(this.m_utils.isTouchDevice() ? this.getMappedStyle("scroller-mobile") : this.getMappedStyle("scroller"));
    }
    else
    {
    ...
    for (i = 0; i < headerCount; i += 1)
    {
        ...
        //do not put borders on last header cell
        if ((this._isLastColumn(index) && this.getRowHeaderWidth()+currentLeft+width >= this.getWidth()-2))
        {
            if (dir === 'left')
            {
...
DvtDataGrid.prototype.buildRowHeaders = function(headerRoot, headerSet, start, totalCount, rowInsert)
{
    var defaultHeight, defaultWidth, headerCount, scroller, resizer, renderer, isAppend, top, totalRowHeight, 
            ...
            col, styleClass, prev, resizableAttribute, content, rowHeaderContent, referenceRow;
    ...
    {
        scroller = document.createElement("div");
        scroller['className'] = this.m_utils.isTouchDevice() ? this.getMappedStyle("scroller-mobile") : this.getMappedStyle("scroller");

        if (headerCount == 0)
        {
            //there is no row headers so do not add any styles to it
    ...
    for (i = 0; i < headerCount; i += 1)
    {
        ...
        headerClass = this.getMappedStyle("headercell") + " " +this.getMappedStyle("rowheadercell");
        if (styleClass != null)
        {
            col['className'] = headerClass + " " + styleClass;
        }
        ...
        if (this._isLastRow(index) && this.getRowBottom(row, top) >= this.getHeight()-2)
        {
    ...
    else if (rowInsert)
    {
        ...
        this.m_endRowHeader = this.m_endRow + headerCount;
 ...
 * @protected
 */
DvtDataGrid.prototype.handleCellsFetchSuccess = function(cellSet, cellRange, rowInsert, animate) //vvc9
{
    ...
    if (!databody.hasChildNodes())
    {
        ...
        databodyContent['className'] = this.m_utils.isTouchDevice() ? this.getMappedStyle("scroller-mobile") : this.getMappedStyle("scroller");
...
DvtDataGrid.prototype._addRows = function(fragment, isAppendOrInsert, top, rowStart, rowCount, columnStart, columnRangeNeedsUpdate, cellSet)
{
    ...
    for (i = 0; i < rowCount; i += 1)
    {
        ...
        {
            row['className'] = this.getMappedStyle("row") + " " + this.getMappedStyle("banded");
        }
        else
        {
            row['className'] = this.getMappedStyle("row");
        ...
        height = this.getRowHeight(row, index, this._getKey(row));
        this.setElementHeight(row, height);
        totalRowHeight = totalRowHeight + height;
...
DvtDataGrid.prototype.pushRowHeadersDown = function(rowHeader, adjustment)
{
    while (rowHeader)
    {
        var top = parseInt(rowHeader['style']['top']) + adjustment;
...
{
    this.pushRowsDown(rowHeader, 0-adjustment);
};
...
{
    var isAppend, firstColumn, inlineStyleClass, cellStyleClass, currentLeft, dir, totalWidth, columnCount, indexes, cellData, cellMetadata, cellContext, j, cell, inlineStyle, width, content, columnIndex, keyAttribute, selectionAffordanceAppend;
    ...
    for (j = 0; j < columnCount; j += 1)
    {
        ...
        if (verticalGridlines === 'hidden' || (this._isLastColumn(columnIndex) && this.getRowHeaderWidth()+currentLeft+width >= this.getWidth()-2))
        {
            if (dir === 'left')
            {
        ...
        if (horizontalGridlines === 'hidden'|| (this._isLastRow(rowIndex) && this.getRowBottom(row, bottom) >= this.getHeight()-2))
        {
...
DvtDataGrid.prototype.handleHeaderMouseOver = function(event)
{
    ...
    this._manageMoveCursor(this.findRow(event.target), true);
};
...
DvtDataGrid.prototype.handleDatabodyMouseUp = function(event)
{
    ...
    this._manageMoveCursor(this.findRow(event.target));
...
DvtDataGrid.prototype.handleHeaderTouchStart = function(event)
{
    ...
    if (fingerCount == 1)
    {
        ...
        setTimeout(function(){
            ...
            {
                this.m_utils.addCSSClassName(header, this.getMappedStyle('active'));
            }
        }.bind(this), 500);
        ...
        setTimeout(function(){
            if (this.m_touchActive && !this.m_isResizing)
            {
                this.m_utils.removeCSSClassName(header, this.getMappedStyle('active'));
            }
        }.bind(this), 1000);
...
{
    var operation, keys, cellSet, indexes, source; //vvc9
 ...
 * @param {boolean=} animate
 */
DvtDataGrid.prototype._handleCellInsertsFetchSuccess = function(cellSet, cellRanges, animate) //vvc9
{
    var rowStart;
    ...
    // insert the row
    this.handleCellsFetchSuccess(cellSet, cellRanges, this.m_endRow > cellRanges[0]['start'], animate); //vvc9
 ...
 * @private
 */
DvtDataGrid.prototype._handleModelInsertRangeEvent = function(cellSet, animate) //vvc9
{
    ...
    // insert the rows
    this._handleCellInsertsFetchSuccess(cellSet, [rowRange, columnRange], animate); //vvc9
};
...
{
    var key, i, rowKey, row, totalHeight, height, referenceRow, databodyContent, scrollerContent, databodyContentHeight, rowHeader;
    ...
    for (i=0; i<keys.length; i++)
    {
        ...
        // delete row or column, keys['row'] can be the number 0
        if (key['row'] != null)
        {
            rowKey = key['row'];
            ...
            if (rowHeader != null)
            {
                ...
                this.m_endRowHeader = this.m_endRowHeader - 1;
                this.m_endRowHeaderPixel = this.m_endRowHeaderPixel - height;            
            }
 ...
 * @private
 */
DvtDataGrid.prototype._handleModelDeleteEventWithAnimation = function(keys) //vvc9
{
    var self = this;
    
    //all inherited animated rows should be hidden under primary collapsed one
    self.changeStyleProperty(this._findRowByKey(keys[0]['row'])['parentNode'], self.getCssSupport('z-index'), 10000);
    setTimeout(function()
    {
        self._collapseRowsWithAnimation(keys, self);
    }, 0);
};
 ...
 * @param {Object} keys the key that identifies the row that got deleted.
 * @param {Object} self the 'this' instance.
 * @private
 */
DvtDataGrid.prototype._collapseRowsWithAnimation = function(keys, self) //vvc9
{
    var key, i, j, k, rowKey, row, totalHeight, height, referenceRow, databodyContent,
        rowHeader, duration, clickedRow, keyAttribute, start, firstRowCase,
    ...
    if(clickedRow.childElementCount == 0)
    {
        clickedRow = self._findRowByKey(keys[0]['row']);
        firstRowCase = true;
    }
    keyAttribute = this.getResources().getMappedAttribute('key');
    for (i=keys.length-1; i>=0; i--)
    {
        ...
        if (key['row'])
        {
            ...
            if (row != null)
            {
                height = self.getElementHeight(row);               
                referenceRow = row['nextSibling'];
                ...
                if(i == keys.length-1)
                {
                    ...
                    {                              
                        ...
                        setTimeout(function()
                        {
                            ...
                            {
                                //row expander collapse animation case
                                ...
                                if(start > 0)
                                {
                                    databodyContent.childNodes[k].style.top = clickedRow.offsetTop + 30*(k-start+1) + 'px';
                                }
                                else
                                {
                                    if(firstRowCase)
                                    {
                                        databodyContent.childNodes[k].style.top = clickedRow.offsetTop + 30*(k-1)+'px';
                                    }
                                }
            ...
            if (rowHeader != null)
            {
                ...
                height = self.getElementHeight(rowHeader);
                referenceRow = rowHeader['nextSibling'];
    ...
    self.pushRowsUpWithAnimation(self, referenceRow, height*keys.length, duration, 'linear');
};
 ...
 * @private
 */
DvtDataGrid.prototype.pushRowsUpWithAnimation = function(self, row, adjustment, duration, easing) //vvc9
{
    ...
    {        
        ...
        self.addUpDownMoveStyle(row, duration + "ms", 0, easing, "-"+adjustment);
...
DvtDataGrid.prototype._handleModelSyncEvent = function(event)
{
    ...
    //fetch the pageSize worth of rows so there is no scroll virtualization in paging
    if (!this.m_initialized)
    {
        this.fetchHeaders("column", 0, this.m_colHeader);
        this.fetchHeaders("row", startRow, this.m_rowHeader, pageSize);
        this.fetchCells(this.m_databody, this.m_scroller, startRow, 0, pageSize);
    } 
    else 
    {
...
DvtDataGrid.prototype.highlightActive = function(focus)
{
    ...
    if (this.m_active != null)
    {   
        cell = this.highlightIndex(this.m_active, "active");
    }
...
DvtDataGrid.prototype.unhighlightIndex = function(index, activeOnly)
{
    ...
    singleCell = cell[0];
    activeClassName = this.getMappedStyle("active");
...
{
    var startRow, endRow, startColumn, endColumn, startKey, endKey, startRowKey, endRowKey, startColumnKey, endColumnKey;
    if (endIndex)
    {
        // -1 means unbound
...
DvtDataGrid.prototype._isFocusableElement = function(elem)
{
    ...
    return (tagName === 'INPUT' || tagName === 'TEXTAREA' || tagName === 'SELECT' || tagName === 'BUTTON' || tagName === 'A' || (elem.getAttribute("tabIndex") != null && parseInt(elem.getAttribute("tabIndex")) >= 0 && this.findCell(elem) != elem));
};
...
DvtDataGrid.prototype.handleHeaderArrowKeys = function(keyCode, event)
{
    ...
    switch (keyCode)
    {
        ...
        case DvtDataGrid.RIGHT_KEY:
            ...
            {
                if (!(index + 1 >= this.m_endColHeader && this.m_stopColumnHeaderFetch) && (this._isCountUnknown("column") || index < this.getDataSource().getCount("column")))
                {
        ...
        case DvtDataGrid.DOWN_KEY:
            ...
            {
                if (!(index + 1 >= this.m_endRowHeader && this.m_stopRowHeaderFetch) && (this._isCountUnknown("row") || index + 1 < this.getDataSource().getCount("row")))
                {
                    this._setActiveHeader(index+1, elem['nextSibling']);
                    this._scrollToActiveHeader();
                    processed = true;
...
{
    var activeClassName = this.getMappedStyle("active");
    ...
    switch (keyCode)
    {
        ...
        case DvtDataGrid.PAGEDOWN_KEY:
            ...
            if (!this._isCountUnknown("column"))
                ...
                newCellIndex = this.createIndex(Math.max(0, this.m_endRow - 1), column);
 ...
 * @private
 */
DvtDataGrid.prototype._getActiveRowKey = function()
{
    if (this.m_active != null)
    {
        //+1 for resizer
        return this._getKey(this.m_databody['firstChild']['childNodes'][this.m_active['row'] + 1 - this.m_startRow]);
    }
    //active row key can also come from an active row header
    else if (this.m_activeHeader != null && this.m_activeHeader['axis'] === 'row')
    {
        return this._getKey(this.m_rowHeader['firstChild']['childNodes'][this.m_activeHeader['index'] + 1 - this.m_startRowHeader]);        
    }
    return null;
};
...
DvtDataGrid.prototype._handleMove = function(event)
{
    ...
    if (this.m_moveRow == null)
    {
        ...
        this.m_utils.addCSSClassName(this.m_moveRow, this.getMappedStyle('move'));
        this.m_originalTop = this.getElementDir(this.m_moveRow, 'top');
  
        this.m_dropTarget = document.createElement("div");
        this.m_utils.addCSSClassName(this.m_dropTarget, this.getMappedStyle('droptarget'));        
        this.setElementHeight(this.m_dropTarget, this.getElementHeight(this.m_moveRow));
        ...
        {
            this.m_utils.addCSSClassName(this.m_moveRowHeader, this.getMappedStyle('move'));        
            this.m_dropTargetHeader = document.createElement("div");
            this.m_utils.addCSSClassName(this.m_dropTargetHeader, this.getMappedStyle('droptarget'));        
            this.setElementHeight(this.m_dropTargetHeader, this.getElementHeight(this.m_moveRowHeader));
    ...
    height = this.getElementHeight(this.m_moveRow);
...
/**
 * Checks to see if the move cursor should be applied or removed from a specific row
 * @param {Element} row the row to move
 * @param {boolean} header is the row in the header
 */
DvtDataGrid.prototype._manageMoveCursor = function(row, header)
{
    if (row != null)
    {
        //if move enabled set the cursor on the row/row header
        if (this._isMoveOnRowEnabled(row))
        {
            ...
            this.m_moveCursor = true;      
        }
        else
        {
            //if move disabled and cursor is move revert to default 
            if (this.m_rowHeader['style']['cursor'] === 'move')
            {
                this.m_rowHeader['style']['cursor'] = 'default';
            }  
            if (row['style']['cursor'] === 'move')
            {
...
DvtDataGrid.prototype.highlightRange = function(range, updateAccInfo)
{
    ...
    if (updateAccInfo)
    {
        this._setAccInfoText('accessibleMultiCellSelected', {'num': elems.length});
    }
};
...
DvtDataGrid.prototype.handleDatabodySelectionDrag = function(event)
{
    ...
    if (this.m_utils.isTouchDevice())
    {    
        cell = this.findCell(document.elementFromPoint(event.touches[0].pageX, event.touches[0].pageY));
    }
    else
    {
        cell = this.findCell(event.target);
...
{    
    var newRange, selection, currentRange, startIndexesMatch, endIndexesMatch, clone;
 ...
 * selection.
 * @param {Element} cell the cell to place the icons on the corners of
 * @private
 */
...
{    
    ...
    if (this.m_topSelectIconContainer == null && this.m_bottomSelectIconContainer == null)
    {
        ...
        else
        {
            ...
            this.setElementDir(this.m_topSelectIconContainer, left, dir);
            this.setElementDir(this.m_bottomSelectIconContainer, left + this.getElementWidth(cell), dir);        
        }
 ...
 * @param {Object} oldSelection the old selection in order to find the icons
 * @private
 */
...
{     
    ...
    if (this.m_topSelectIconContainer != null && this.m_bottomSelectIconContainer != null)
    {
        ...
        else
        {
            ...
            this.setElementDir(this.m_bottomSelectIconContainer, this.getElementDir(bottomIconCell, dir) + this.getElementWidth(bottomIconCell)  - (this._getTouchSelectionAffordanceSize()/2), dir);
        }
    ...
    if (this.m_isResizing === false)
    {
        ...
        if (this.m_resizingElement != null)
        {
            ...
            {                
                ...
                {
                    header['style']['cursor'] = this.m_moveCursor === true ? 'move':this.m_cursor;
                }
                else
                {
...
DvtDataGrid.prototype.manageResizeScrollbars = function() {
    var width, height, colHeader, rowHeader, scroller, databody, colHeaderHeight, rowHeaderWidth, 
        ...
        isDatabodyVerticalScrollbarRequired, scrollbarSize, dir, borderWidth, scrollerHeight, scrollerWidth;
...
{
    var me, pos, oldLength, oldSet, newSet, animOrder, restSet, v, gridHeight, colHeaderHeight, gridBodyHeight, oldRowHeaderElements,
        ...
        newElementSetClone, newRowHeaderElementsClone;
    ...
    rowHeight = this.getDefaultRowHeight();
    offset = parseInt(oldElementSet.childNodes[1].style.top.split("px")[0]);
    if (!offset)
    {
        offset = 0;
    }
    ...
    for (l = 0; l < oldElementSet.childElementCount; l++)
    {
        ...
        {
            pos = parseInt(animOrder[oldSet[l]].split('_')[1]) * rowHeight;
