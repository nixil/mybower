...
DvtGaugeDataUtils.getValueThresholdIndex = function(gauge, value) {
  var options = gauge.__getOptions();
  var gaugeValue = value ? value : options['value'];
...
DvtGaugeKeyboardHandler.prototype.processKeyDown = function(event) {
  ...
  // don't call superclass so that the arrow key behaviors are not overridden
};
...
DvtStatusMeterGaugeRenderer._renderShape = function(gauge, container, bounds) {
  ...
  if (isVert) {
    ...
    plotY1 = Math.min(bounds.y, Math.min(bounds.y + bounds.h, plotAreaEnd));
    ...
    plotX2 = Math.max(bounds.x, Math.max(bounds.x + bounds.w, plotAreaEnd));
    plotY1 = bounds.y;
  ...
  if (thresholds && options['plotArea']['rendered'] != 'off' && options['thresholdDisplay'] == 'all') {
    for (var i = thresholds.length - 1; i >= 0; i--) {
      ...
      if (i == thresholds.length - 1) {
        ...
        if (isVert) {
          cp.addRect(0, axisInfo.getUnboundedCoordAt(options['max']), plotX2 - plotX1 + 2, (Math.abs(options['max'] - thresholds[thresholds.length - 2 - currentThresholdIndex]['max'])) * 1 / (Math.abs(options['min'] - options['max'])) * bounds.h, 0, 0);
        }
        else {
          if (isRTL) {
...
DvtStatusMeterGaugeRenderer._renderLabel = function(gauge, container, bounds) {
  ...
  // Allocate space for the horizontal label
    ...
    var longestLabel = Math.max(maxLabel.toString().length, minLabel.toString().length);
    var maxString = '';
    if (options['metricLabel']['textType'] == 'percent') {
      longestLabel = 3;
      maxString += '%';
...
DvtRatingGaugePeer.prototype.getDatatip = function(target, x, y) {
  var options = this._gauge.__getOptions();
  var thresholdIndex;
