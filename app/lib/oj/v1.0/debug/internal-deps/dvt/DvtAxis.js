...
DvtAxisRenderer._renderLabelsHoriz = function(axis, axisInfo, availSpace) {
  ...
  for (var i = 0; i < labels.length; i++) {
    ...
    if (axisInfo.isLabelRotated()) {
      ...
      if (DvtTextUtils.guessTextDimensions(label).h > availSpace.h)
        continue;
...
DvtTimeAxisInfo.prototype._getMixedFrequencyStep = function() {
  ...
  if (this._averageInterval >= DvtTimeAxisInfo.TIME_MONTH_MAX)
...
DvtTimeAxisInfo._getLabelPositions = function(start, end, step) {
  ...
  var anchor = new Date(new Date(start).getFullYear(), 0, 1, 0, 0, 0).getTime(); // beginning of year
  var time = anchor + Math.ceil((start - anchor) / step) * step;
  var times = [];
  while (time <= end) {
    times.push(time);
    time += step;
 ...
 * @private
 */
DvtTimeAxisInfo.prototype._skipLabelsUniform = function(labels, labelDims, minSkip) {
