...
oj.CollectionDataGridDataSource.prototype._handleCellFetchSuccess = function(cellRanges, callbacks, callbackObjects, actualRange)
{
    ...
    {
        rowEnd = Math.min(this.size(), rowStart + cellRanges['count']);        
    }
...
oj.CollectionDataGridDataSource.prototype._fetchCells = function(cellRanges)
{
    ...
    this.collection.setRangeLocal(rowStart, rowCount).done(function (start, count)
    {
        var first = this.collection.at(rowStart, {'deferred':true});
        
        this._setActualCallbackRanges(start, count);
        
        // check if we need to poach columns from row
        if (first != null && this.columns === undefined)
        {
            first.done(function(model){this._setupColumns(model)}.bind(this));
        }
 ...
 * @return {Object} a JQuery Deferred object which upon resolution will pass in an object containing the keys for each axis
 * @export
 */
oj.CollectionDataGridDataSource.prototype.keys = function(indexes)
{
    var rowIndex, columnIndex, rowKey, columnKey, dfd, atPromise;
    rowIndex = indexes['row'] + this._startIndex;
    columnIndex = indexes['column'];

    dfd = $.Deferred();

    atPromise = this.collection.at(rowIndex, {deferred: true});
    //.at() will return null if index known to be OOB
    if (atPromise != null)
    {
        atPromise.done(function(rowModel) {
            rowKey = oj.CollectionDataGridUtils._getModelKey(rowModel);
            if (this.columns == null)
            {
                this._setupColumns(rowModel);
            }
            columnKey = this.columns[columnIndex];
            dfd.resolve({"row": rowKey == null ? null : rowKey, "column": columnKey == null ? null : columnKey});
        }.bind(this));
    }
    else
    {
        dfd.resolve({"row": null , "column": null});
    }
    return dfd;
};
 ...
 * @return {Object} a JQuery Deferred object which upon resolution will pass in an object containing the indexes for each axis
 * @export
 */
oj.CollectionDataGridDataSource.prototype.indexes = function(keys)
{
    var rowKey, columnKey, dfd, columnIndex;
    rowKey = keys['row']; 
    columnKey = keys['column'];
    
    dfd = $.Deferred();

    this.collection.indexOf(rowKey, {deferred:true}).done(function(rowIndex){
        if (this.columns == null)
        {
            this.collection.first(1, {'deferred':true}).done(function(model){this._setupColumns(model);}.bind(this));
        } 
        columnIndex = this.columns.indexOf(columnKey);
        dfd.resolve({"row": rowIndex == null ? -1:rowIndex, "column": columnIndex == null ? -1:columnIndex});
    }.bind(this));
...
{
    var indexPromise;
    this.collection.get(moveKey, {deferred: true}).done(
        function(moveModel) {
            if (atKey == null)
            {
            ...
            else
            {
                ...
                indexPromise.done(function(newIndex) {
...
{
    var event, rowKey;
    this.collection.indexOf(model, {deferred: true}).done(function(index) 
    {
        ...
        if (index < this._pageSize + this._startIndex)
        {
            ...
            if (atPromise != null)
            {
                atPromise.done(function(overfillModel) {
                    if (model != null)
                    {
                        this._handleModelDeleted(overfillModel);
                    }
                }.bind(this));
            }
        }            
