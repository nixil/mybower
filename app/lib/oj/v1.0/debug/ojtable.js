define(['ojs/ojcore', 'jquery', 'ojs/ojcomponentcore', 'ojs/ojmenu', 'ojs/ojdatacollection-common', 'ojs/ojpagingcontrol'], 
       /*
        * @param {Object} oj 
        * @param {jQuery} $
        */
...
oj.FlattenedTreeRowSet.prototype.fetch = function(options)
{
  ...
  var rangeOption = {'start': this._startIndex, 'count':  rangeCount};
...
{ 
  var range = this._data.getFetchedRange();
...
oj.FlattenedTreeRowSet.prototype.sort = function(criteria)
{
  ...
  return this._data.getWrappedDataSource().sort(criteria,
    {"success": function(nodeSet)
      {
        this._handleFetchRowsSuccess(nodeSet);
        oj.FlattenedTreeRowSet.superclass._handleEvent.call(this, oj.RowSet.EventType['SORT'], null);
      }.bind(this),
      "error": function(status)
      {
        //this._handleFetchRowsError(status, {'start': rowStart, 'count': rowCount}, callbacks, callbackObjects);
...
oj.PagingTableDataSource.prototype.next = function()
{
  if (this.dataSource.totalSize() > this._startIndex)
  {
    ...
    this.fetch({'startIndex': this._currentStartIndex, 'pageSize': this._pageSize, 'fetchType': 'next'});
  }
};
...
oj.PagingTableDataSource.prototype.previous = function()
{
  if (this._startIndex != 0 || this._startIndex != -1)
  {
    ...
    this._currentPageSize = this._currentPageSize + this._pageSize;
    this.fetch({'startIndex': this._startIndex, 'pageSize': this._currentPageSize, 'fetchType': 'previous'});
  }
};
...
oj.PagingTableDataSource.prototype.on = function(eventType, eventHandler)
{
  ...
  {
    return dataSource.on(eventType, function(event){self._handleSyncEvent(event, eventHandler);});
  }
  else
  {
...
{
  if (event['fetchType'] == 'next')
  {
    var dataSource = (/** @type {{handleEvent: Function}} */ (this.dataSource));
    ...
    {
      dataSource.handleEvent(oj.RowSet.EventType['ADD'], this.dataSource.at(i));
    }
  }
  else
  {
  ...
  oj.__registerWidget("oj.ojTable", $['oj']['baseComponent'],
    {
      ...
      _LOGGER_MSG:
        {
          _ERR_PREACTIVEROW_ERROR_SUMMARY:                'Did not change active row due to error.',
          _ERR_PREACTIVEROW_ERROR_DETAIL:                 'Error detail: {error}.',
          _ERR_ACTIVEROW_UNAVAILABLE_INDEX_SUMMARY:       'Did not change active row due to unavailable row index.',
          _ERR_ACTIVEROW_UNAVAILABLE_INDEX_DETAIL:        'Unavailable row index: {rowIdx}.',
       ...
       * the current active row index. If called with -1 then resets the activeRow.
       * If index argument is specified then sets the active row. A preactiverow 
      ...
      'activeRow': function(index)
      {
        ...
        else
        {
          if (index != -1 && 
              ...
              index < -1))
          {
            var errSummary = this._LOGGER_MSG._ERR_ACTIVEROW_UNAVAILABLE_INDEX_SUMMARY;
            var errDetail = oj.Translations.applyParameters(this._LOGGER_MSG._ERR_ACTIVEROW_UNAVAILABLE_INDEX_DETAIL, {'rowIdx': index});
            throw new Error(errSummary + '\n' + errDetail);
          }
          if (this._activeRowIndex != index)
          {
            ...
            catch (err)
            {
              ...
              var errSummary = this._LOGGER_MSG._ERR_PREACTIVEROW_ERROR_SUMMARY;
              var errDetail = oj.Translations.applyParameters(this._LOGGER_MSG._ERR_PREACTIVEROW_ERROR_DETAIL, {'error': err.toString()});
              throw new Error(errSummary + '\n' + errDetail);
            }
        ...
        if (selection != null)
        {
          ...
          for (i = 0; i < selection.length; i++)
          {
            ...
            else
            {
              ...
              console.log('start row: ' + startRowIdx);
              console.log('end row: ' + endRowIdx);
              console.log('start column: ' + startColumnIdx);
              console.log('end column: ' + endColumnIdx);
              return null;
            }
          }
      ...
      {
        var self = this;
        $.when(/** @type {jQuery.deferred} */ (row)).done(function(resolvedModel)
                 {
                   setTimeout(function()
                   {
      ...
      {
        var i, tableBodyRows, tableBodyRow;
        for (i = 0; i < rows.length; i++)
        {
          tableBodyRows = this._getTableDomUtils().getTableBodyRows();
          
          if (tableBodyRows != null && 
              rows[i].rowIdx < tableBodyRows.length && 
              rows[i].rowIdx >= 0)
          {
            tableBodyRow = $(tableBodyRows[rows[i].rowIdx]);
      ...
      _refreshTableBody: function()
      {
        ...
        else
        {
          ...
          var i, j, row, rowIdx, columns, tableBodyRow;
      ...
      {         
        if (!this._isResizeListenerAdded)
        {
          ...
          oj.DomUtils.addResizeListener(element[0], function(width, height)
                                                    {
                                                      self._getTableDomUtils().refreshTableDimensions(false, width, height);
                                                    });
...
oj.TableDomUtils.prototype.createContextMenu = function(handleContextMenuBeforeShow, handleContextMenuSelect)
{
  ...
  if (this.options["contextMenu"] == null && this.getTable().attr("contextmenu") == null)
  {
    ...
    menuContainer.attr('id', this.getTable().attr('id') + 'contextmenu');
...
oj.TableDomUtils.prototype.createTableHeaderColumn = function(columnIdx, columnSelectionMode)
{
  ...
  if (column.sortable == oj.TableDomUtils._OPTION_ENABLED)
  {
    ...
    {
      headerColumnRowDiv.css('padding-left', '0px');
    }
    else
    {
      headerColumnRowDiv.css('padding-right', '0px');
    }
  }
  ...
  headerColumn.attr('title', column.headerText);
  ...
  headerColumnTextDiv.css('display', 'inline-block');
...
oj.TableDomUtils.prototype.getTableContainerDimensions = function()
{
  ...
  var tableHeight = tableContainer.height() > 1 ? tableContainer.height() : 0;
  var tableWidth = tableContainer.width() > 1 ? tableContainer.width() : 0;
...
oj.TableDomUtils.prototype.styleTableBodyCell = function(columnIdx, tableBodyCell)
{
  ...
  {
    if (!tableBodyCell.hasClass(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_VGRID_LINES_CLASS))
    {
  ...
  {
    if (!tableBodyCell.hasClass(oj.TableDomUtils.CSS_CLASSES._TABLE_DATA_CELL_HGRID_LINES_CLASS))
    {
 ...
 oj.TableDomUtils.prototype._refreshTableDimensions = function(width, height)
{
  ...
  if (!this._specifiedTableStyle)
  {
    this._specifiedTableStyle = table.attr('style');
  }

  if (!this._tableContainerDimensions)
  {
    table.attr('style', this._specifiedTableStyle);
    this._tableContainerDimensions = this.getTableContainerDimensions();
    
    if (this._tableContainerDimensions['height'] != 0 && !this.isDivScroller())
    {
      // stretch the table to the container
  ...
  if (!this.isDivScroller())
  {
    ...
    if (isTableWidthScrolled)
    {
      if (data != null && data.size() > 0)
      {
        ...
        tableBody.css('width', this._tableContainerDimensions['width']);
        if (tableHeader != null)
        {
          tableHeader.css('width', this._tableContainerDimensions['width']);
        }
      }
      else
      {
    ...
    if (isTableHeightScrolled)
    {
      ...
      var tableBodyHeight = this._tableContainerDimensions['height'] - tableHeaderHeight - tableFooterHeight - captionHeight;
  ...
  if (!isTableWidthScrolled)
  {
    ...
    if (this._tableContainerDimensions['width'] == 0)
    {
      // if width is not specified and there is not width scrolling then
      // size everything to the tbody width
      tableBody.css('display', 'inline-block');
      if (tableBody.width() > 0)
      {
        table.css('width', tableBody.width() + 'px');
        tableContainer.css('width', tableBody.width() + 'px');
      }
      else if (data != null && data.size() > 0 && tableBody.children().length > 0)
      {
        // this means we have rows but we need to refresh again later once the
        // the browser have fully rendered the rows
        this._refreshTableDimensionsTimer = {width: width, height: height};
        return;
      }
      tableBody.css('display', 'block');
  ...
  if (this._tableContainerDimensions['height'] == 0 && !this.isDivScroller())
  {
    ...
    // We need to manually do this because the thead/tfoot has absolute positioning
  ...
  if (this.isDivScroller())
  {
    ...
    if (isTableHeightScrolled)
    {
      tableDivScroller.css('height', this._tableContainerDimensions['height']);
    }
...
oj.TableDomUtils.prototype._refreshTableMessagingPosition = function()
{
  ...
  if (tableHeader != null)
  {
    ...
    var tableHeaderOffsetTop = tableHeader[0].offsetTop;
    tableNoDataMessage.css('top', tableHeaderHeight + tableHeaderOffsetTop + 'px');
  }
  else
  {
 ...
 * @private
 */
oj.TableDomUtils.prototype._removeTableBodyCellBottomBorder = function()
{
  ...
  if (tableBodyRows != null && tableBodyRows.length > 0)
  {
    ...
    var tableBodyCells = this.getTableBodyCells(tableBodyRows.length - 1, lastTableBodyRow);
    ...
    if (tableBodyCells != null && tableBodyCells.length > 0)
    {
      ...
      var borderBottomWidth = parseInt($(tableBodyCells[0]).css('borderBottomWidth'), 10);
    
      if (lastTableBodyRowRect.bottom + borderBottomWidth == tableRect.bottom)
      {
        var i;
        for (i = 0; i < tableBodyCells.length; i++)
        {
          if ($(tableBodyCells[i]).css('border-bottom') != '0')
          {
            $(tableBodyCells[i]).css('border-bottom', '0');
          }
        }
      }
    }
...
oj.TableDomUtils.prototype._setColumnWidths = function()
{
 ...
 var i, headerColumn, headerColumnRowDiv, headerColumnTextDivHeight, headerColumnTextDiv, footerCell;
 for (i = 0; i < columns.length; i++)
 {
   headerColumn = this.getTableHeaderColumn(i);
   if (headerColumn != null)
...
oj.TableDomUtils.CSS_CLASSES =
  {
    ...
    _COLUMN_HEADER_ROW_CLASS: 'oj-table-column-header',
