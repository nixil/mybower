  ...
  var _ComboUtils = {
    ...
    local: function(options, keys)
    {
      var data = options, // data elements
          ...
          text = function (item)
            {
              return "" + item[keys['labelKey']] || item[keys['optgroupLabelKey']];
            };
...
//        this.search = search = this.container.find("input." + className + "-input");
      ...
      _prepareOpts : function (opts)
      {
        ...
        // set up options keys
        keys['valueKey'] = (opts['optionKeys'] && opts['optionKeys']['value']) ? opts['optionKeys']['value'] : 'value';
        keys['labelKey'] = (opts['optionKeys'] && opts['optionKeys']['label']) ? opts['optionKeys']['label'] : 'label';
        keys['optgroupLabelKey'] = (opts['optgroupKeys'] && opts['optgroupKeys']['label']) ? opts['optgroupKeys']['label'] : 'label';
                
        opts.id = function (e)
        {
          return e[keys['valueKey']];
        };
        ...
        opts.formatResult = function (result, escapeMarkup)
        {
          var markup = [], label;
          if (result.children)
            label = result[keys['optgroupLabelKey']];
          else 
            label = result[keys['labelKey']];
        ...
        {
          return (data && data[keys['labelKey']]) ? escapeMarkup(data[keys['labelKey']]) : undefined;
        };
        ...
        if (tagName !== "select")
        {
          ...
          {
            var entry = {};
            entry[keys['valueKey']] = entry[keys['labelKey']] = $.trim(term);
            return entry;
          }
        }
        ...
        {
          opts.query = _ComboUtils.local(opts.options, keys);
        }
      ...
      _updateResults : function (initial)
      {
        ...
        opts.query(
        {
          ...
          callback : this._bind(function (data)
          {
            ...
            if (data.results.length === 0 && opts.manageNewEntry && search.val() !== "")
            {
              def = opts.manageNewEntry.call(self, search.val(), data.results);
              if (def !== undefined && def !== null && self.id(def) !== undefined && self.id(def) !== null && self.id(def) !== "")
              {
  ...
  var _OjSingleCombobox = _ComboUtils.clazz(_AbstractSingleChoice,
    {
        ...
        var container = $(document.createElement("div")).attr(
          {
            "class" : "oj-combobox oj-component"
              ...
              "   <abbr class='oj-combobox-search-choice-close' role='presentation'></abbr>",
      ...
      _updateSelectedOption : function(selected)
      {
        ...
        //No need to call close() as by the time _updateSelectedOption is invoked popup is already closed. Not very sure, Do we need to?? 
        //But there is no harm in callling this as it does nothing when popup is closed
        this.close();
      }
    }
    );
  ...
  var _OjSingleSelect = _ComboUtils.clazz(_AbstractSingleChoice,
    {
         ...
         if (selected !== undefined && selected !== null)
            {
              ...
              if(value !== selectedVal)
                this.ojContext.options['value'] = Array.isArray(value)? [selectedVal]:selectedVal;
      ...
      _updateSelection : function (data)
      {
        ...
        {
          var optionLabel;
          // get text key from optionKeys if it exists
          // otherwise use default
          if (this.opts['optionKeys'] && this.opts['optionKeys']['label'])
            optionLabel = data[this.opts['optionKeys']['label']];
          else
            optionLabel = data['label'];
            
          this.text.text(optionLabel);
        }
        ///ojselect placeholder
        ///reduce number of call to setVal
...
*/
      ...
      _hasSearchBox : function ()
      {
        ...
        var len = this.datalist ? this.datalist[0].length : this.opts.options.length;
 ...
 */
  ...
  oj.__registerWidget("oj.ojCombobox", $['oj']['editableValue'],
  {
    ...
    options :
    {
       ...
       * array containing the option items.
       *
       * @expose
       * @memberof! oj.ojCombobox
       * @instance
       ...
       * $( ".selector" ).ojCombobox( { "options": [{value: 'option1', label: 'option1'}, {value: 'option2', label: 'option2'}, {value: 'option3', label: 'option3'}] } );
       ...
       * @type {Object}
       *
       * @example <caption>Initialize the Combobox with <code class="prettyprint">optionKeys</code> specified. This allows the key names to be redefined in the options array.</caption>
       * $( ".selector" ).ojCombobox( { "optionKeys": {value : "state_abbr", label : "state_name"} } );
       */
      optionKeys : null,
      
       /**
       * Specify the key names to use in the options array for optgroup.
       *
       * @expose
       * @memberof! oj.ojCombobox
       * @instance
       * @type {Object}
       *
       * @example <caption>Initialize the Combobox with <code class="prettyprint">optgroupKeys</code> specified. This allows the key names to be redefined for the optgroup in the options array.</caption>
       * $( ".selector" ).ojCombobox( { "optgroupKeys": {label: "regions"} } );
       */
      optgroupKeys : null,
    ...
    getNodeBySubId: function(locator)
    {
      ...
      if (!node)
      {
        ...
        if (subId === "oj-combobox-selection")
          subId = "oj-combobox-search-choice";
  ...
  * </p>
  ...
  oj.__registerWidget("oj.ojSelect", $['oj']['editableValue'],
  {
    ...
    options :
    {
      ...
      // TODO: revisit
      // Please talk about the special things you do when placeholder is set. 
      // - If the placeholder option is specified to a string, you should no longer include an empty 
      // option as your first option for the dropdown. When the dropdown is open, ojselect adds a 
      // placeholder item at the beginning of the dropdown list with
      //  - displayValue: placeholder text 
      //  - value: an empty string
      //  The placeholder item in the dropdown is selectable. However, it's not a valid choice, i.e. 
      //  validation will fail if the select component is a required field
      //  The placeholder item doesn't participate in the filtering, so it will not appear in the 
      //  result list with a filter specified.      
      // provide examples for the above.
       ...
       * @type {Object}
       *
       * @example <caption>Initialize the Select with <code class="prettyprint">optionKeys</code> specified. This allows the key names to be redefined in the options array.</caption>
       * $( ".selector" ).ojSelect( { "optionKeys": {value : "state_abbr", label : "state_name"} } );
       */
      optionKeys : null,
       ...
       * @type {Object}
       *
       * @example <caption>Initialize the Select with <code class="prettyprint">optgroupKeys</code> specified. This allows the key names to be redefined for the optgroup in the options array.</caption>
       * $( ".selector" ).ojSelect( { "optgroupKeys": {label: "regions"} } );
       */
      optgroupKeys : null,
    ...
    _setup : function ()
    {
      ...
      //need to change when placeholder is removed from editableValue
      if (this._placeHolderTemp)
      {
        this.select.opts.placeholder = this._placeHolderTemp;
        this._placeHolderTemp = undefined;
    ...
    _SetPlaceholder : function(value)
    {
      ...
      //TODO: select is not initialized yet, setPlaceholder in _setup later
      //need to change when placeholder is removed from editableValue
      else
      {
        this._placeHolderTemp = value;
      }
    },
...
*/
     ...
     *       the locator object should have the following:
     *          subId: 'oj-listbox-result-label'
     *          index: number
     *
     * @expose
     * @memberof! oj.ojSelect
  ...
  var _OjMultiCombobox = _ComboUtils.clazz(_AbstractOjChoice,
    {
      ...
      _addSelectedChoice : function (data)
      {
        ...
        enabledItem = $(
            ...
            "    <div></div>" +
            "    <a href='#' onclick='return false;' class='" + this._classNm + "-search-choice-close' tabindex='-1'>" +
...
//TODO: translation string
      ...
      setVal : function (val, event)
      {
        ...
        this.ojContext._SetValue(unique, event);
