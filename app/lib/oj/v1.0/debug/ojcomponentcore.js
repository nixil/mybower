...
$.widget('oj.baseComponent', 
{
   ...
   */
  /**
   * This method has several overloads, which gets and set component options.  The functionality is unchanged from 
   * that provided by JQUI.
   * 
   * <p>The first overload accepts a single <code class="prettyprint">optionName</code> param as a string, and returns 
   * the current value of that option.  For object-valued options, dot notation can be used to get the value of a field 
   * or nested field.
   * 
   * <p>The second overload accepts two params, an <code class="prettyprint">optionName</code> string and a new value to 
   * which that option will be set.  For object-valued options, dot notation can be used to set the value of a field 
   * or nested field, without altering the rest of the object.
   * 
   * <p>The third overload accepts no params, and returns a map of key/value pairs representing all the component 
   * options and their values.
   * 
   * <p>The fourth overload accepts a single map of option-value pairs to set on the component.  Unlike the first two 
   * overloads, dot notation cannot be used.
   * 
   * @expose
   * @memberof! oj.baseComponent
   * @instance
...
function _mergeObjectsWithExclusions(target, input, ignoreSubkeys, basePath)
{
  ...
  {
    for (var key in input[inputIndex])
    {
      ...
      {
        var value = input[inputIndex][key];
...
/**
 * Returns the node that is represented by the locator.
 * 
 * @param {string} subId
 * @param {number=} index
 ...
 * @private
 */
oj.ComponentMessaging.prototype.getNodeBySubId = function (subId, index)
{
  var targetNode = null;
  $.each(this._strategies, function(i, strategy) 
    {
      if (!targetNode)
      {
        // delegate to strategies to locate target node by locator
        targetNode = strategy.getNodeBySubId(subId, index);
      }
    });  
 ...
 * @private
 */
oj.MessagingStrategy.prototype.getNodeBySubId = function (subId, index)
{
  return null;
};
 ...
 * @private
 */
oj.DefaultMessagingStrategy.prototype.update = function (content)
{
  ...
  oj.DefaultMessagingStrategy.superclass.update.call(this, component, launcher, content);
 ...
 * @private
 */
oj.DefaultMessagingStrategy.prototype.deactivate = function (content)
{
  ...
  oj.DefaultMessagingStrategy.superclass.deactivate.call(this, content);
};
...
/**
 * helper to determine if we have invalid messages among the list of messages.
 * 
