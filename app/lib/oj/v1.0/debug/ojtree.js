...
//Fri28 this._emitEvent(this._data.core.prepared_move, "move");
      ...
      /**
        ...
        * Can be a node or -1 to clean the whole tree.
        * @private
        */
...
//JRM       .not(".oj-tree-open").removeClass("oj-tree-leaf").addClass("oj-tree-closed");
         obj.not(".oj-tree-open, .oj-tree-closed")
            .addClass(OJT_LEAF)
            .children("ul")
            .remove();
    ...
    _select : function (node, check, e)
    {
     ...
     _deselectAll : function(context, bSeparate)
     {
      ...
      /**
        ...
        *
        *  Refreshes the tree - notes all open nodes, reloads them the tree
        *  and then reopens the notes open nodes.
        *  @private
        */
...
//                      tmp.prepend("<ins class='oj-tree-icon'>&#160;</ins><span class='" + OJT_TITLE + "'>" + title + "</span>");
                        ...
                        tmp.prepend("<ins class='" + OJT_ICON + "'>&#160;</ins>", sp);
                        bIns = true ;
                      }
                      if (!m["icon"] && js["icon"])  {
                        m["icon"] = js["icon"];
...
//     },
     ...
     _dnd_prepare : function ()
     {
       ...
       if (this._isRtl) {
         ...
         this._data.dnd.off.right    = this._data.dnd.off.left + vars.r.width() - vars.r.find(">a").width() -18 ;
       }
...
//   this._data.dnd.targ_ml_width = this._data.dnd.targ_ml_width < 25? 25   : this._data.dnd.targ_ml_width ;
     ...
     _dnd_show : function ()
     {
           ...
           { 
             if (this._data.dnd[val])  {
               ctl.helper.children("ins")
                         .removeClass(OJT_DRAG_INVALID)
                         .addClass(OJT_DRAG_OK);
               r = val;
               return false;
             }
        }, this));
        ...
        if (r === false)  {
           ctl.helper.children("ins")
                     .removeClass(OJT_DRAG_OK)
                     .addClass(OJT_DRAG_INVALID);
        }
...
//mon20                                this._data.core.li_height - 6) + "px" }).show();
                                       this._data.core.li_height - 8) + "px" 
                           }).show();
                if (vars.ml) {
                    vars.ml.css({ "left" : mlLeft + "px",
                                  "top" : (this._data.dnd.off.top +
//mon20                                    this._data.core.li_height - 1) + "px" }).show();
                                           this._data.core.li_height - 3) + "px" 
                                }).show();
                }
                break;
            ...
            case "inside":
                  vars.m.css({ "left" : mLeft + (this._isRtl? (isParent? -4 : 0) : 4) + "px",
                               "top" : (this._data.dnd.off.top + this._data.core.li_height/2 - 5) + "px"
                              }).show();
                if (vars.ml)  {
                  vars.ml.hide();
                }
...
//       return func.old.apply(this, (replace_arguments ? Array.prototype.slice.call(arguments, 1) : args ) );
     ...
     _start : function()
     {
       ...
       this._data.core.li_height = this._$container_ul.find("li.oj-tree-closed, li.oj-tree-leaf")
                                                      .eq(0)
                                                      .height() || 18;
...
/* TDO
                        if (! this._data.core.initFocus) {
                          this._data.core.initFocus = true ;
                          this["select"](this._$container_ul.find("li:first"));
                        }
*/
                      }, this)
               )
...
//Wed3                      if(this.parentNode)
     ...
     _initDnD : function()
     {
        ...
        vars.m = $("<div />")              // dnd insertion point marker
            .addClass(OJT_MARKER)
            .hide()
            .html("&raquo;")
        ...
        if (this._isRtl) {
          vars.m.addClass("oj-tree-marker-rtl") ;
        }
...
/*
   // load the css when DOM is ready  For review
   $(function() {
       var  css = '' + 
