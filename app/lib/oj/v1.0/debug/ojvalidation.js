...
OraDateTimeConverter =  (function() {
  ...
  var _TOKEN_REGEXP = /ccccc|cccc|ccc|cc|c|EEEEE|EEEE|EEE|EE|E|dd|d|MMMMM|MMMM|MMM|MM|M|LLLLL|LLLL|LLL|LL|L|yyyy|yy|y|hh|h|HH|H|KK|K|kk|k|mm|m|ss|s|aa|a|SS|S|zzzz|zzz|zz|z|GGGGG|GGGG|GGG|GG|G/g;
...
OraNumberConverter = (function () {
  ...
  _LENIENT_REGEX_PARSE_FLOAT = /(\D*)([+\-]?\d*\.?\d*(E[+\-]?\d+)?)/,
  _ESCAPE_REGEXP = /([\^\$\.\*\+\?\|\[\]\(\)\{\}])/g,
 ...
 * Example:<p>
 * '\\d{10}'</li>
 * <li><b>hint</b>: an optional hint text. There is no default hint provided by the validator. It is 
 * generally not recommended to show the actual pattern in the hint as it might be confusing to 
 * end-user.<p>
 * Parameters: <p>
 ...
 * <li>
 * <b>messageSummary</b>: a custom error message summarizing the error. It is generally not 
 * recommended to show the actual pattern in the hint as it might be confusing to end-user. <p>
 * Parameters: <p>
 ...
 * the users input does not match the specified pattern.  It is generally not recommended to show 
 * the actual pattern in the hint as it might be confusing to end-user.<p>
 * Parameters:<p>
...
oj.DateTimeRangeValidator.prototype.Init = function (options)
{
  ...
  //setting to null for the default validator [min + max option] is taken care of for ojInputDate in _InitOptions for min + max values [default validator]; however 
 ...
 *
 * @param {Date} value that is being validated
 * @returns {Date} original if validation was successful
 *
 * @throws {Error} when there is no match
 * @export
 */
