...
*/
	...
	/////////////////////////////////////////////////////////////////////////////////
	// 
	// Code block for containment functionality (formelry defined as a plugin)
	// 
...
*/
    ...
    *
    * <p>Three formatting classes are provided:
    *
    *<ul>
    ...
    *</ul>
    *
    * For automically created headers, the title of the header is the dialog title, and a close button is created.
    *
    *<h3>Focus</h3>
    *
    ...
    *  <li>The first <a href="/tabbable-selector/"><code>:tabbable</code></a> element within the dialog body</li>
    *  <li>The first <a href="/tabbable-selector/"><code>:tabbable</code></a> element within the dialog footer</li>
   ...
   // * @memberof oj.ojDialog
	...
	_AfterCreate: function() {
	    ...
	    if (this._cssMinWidth != "none") {
		tempE.width(this._cssMinWidth);
		this._cssMinWidthNumeric = tempE.width();
		if (this._isNumber(this._cssMinWidthNumeric))
		    this._cssMinWidthNumeric = Math.round(this._cssMinWidthNumeric);
	    } 
	    else this._cssMinWidthNumeric = "none";

	    this._cssMaxWidth = this.uiDialog.css('max-width');
	    if (this._cssMaxWidth != "none") {
		tempE.width(this._cssMaxWidth);
		this._cssMaxWidthNumeric = tempE.width();
		if (this._isNumber(this._cssMaxWidthNumeric))
		    this._cssMaxWidthNumeric = Math.round(this._cssMaxWidthNumeric);
	    } 
	    else this._cssMaxWidthNumeric = "none";

	    this._cssMinHeight = this.uiDialog.css('min-height');
	    if (this._cssMinHeight != "none") {
		tempE.height(this._cssMinHeight);
		this._cssMinHeightNumeric = tempE.height();
		if (this._isNumber(this._cssMinHeightNumeric))
		    this._cssMinHeightNumeric = Math.round(this._cssMinHeightNumeric);
	    } 
	    else this._cssMinHeightNumeric = "none";

	    this._cssMaxHeight = this.uiDialog.css('max-height');
	    if (this._cssMaxHeight != "none") {
		tempE.height(this._cssMaxHeight);
		this._cssMaxHeightNumeric = tempE.height();
		if (this._isNumber(this._cssMaxHeightNumeric))
		    this._cssMaxHeightNumeric = Math.round(this._cssMaxHeightNumeric);
	    } 
	    else this._cssMaxHeightNumeric = "none";
	    ...
	    if (this._cssWidth != "auto") {
		tempE.height(this._cssWidth);
		this._cssWidthNumeric = tempE.height();
		if (this._isNumber(this._cssWidthNumeric))
		    this._cssWidthNumeric = Math.ceil(this._cssWidthNumeric);
	    } 
	    else this._cssWidthNumeric = "auto";
...
*/
	...
	_createCloseButton: function(domDestination) {
		...
		keydown: function( event ) {
                    ...
                    if (event.keyCode && event.keyCode === $.ui.keyCode.SPACE) {
			event.preventDefault();
			this.close( event );
			return;
		    }
	...
	_makeResizable: function() {
	    ...
	    var AminHeight = this._minHeight();
	    var AmaxHeight = this._cssMaxHeightNumeric;

	    var AminWidth = this._cssMinWidthNumeric;
	    var AmaxWidth = this._cssMaxWidthNumeric;
		...
		maxHeight: this._cssMaxHeightNumeric,

		minWidth: this._cssMinWidthNumeric,
		maxWidth: this._cssMaxWidthNumeric,
		...
		stop: function( event, ui ) {
		    this._cssHeightNumeric = $( this ).height();
		    this._cssWidthNumeric = $( this ).width();
	...
	_minHeight: function() {
	    ...
	    if (this._cssHeightNumeric === "auto") 
		minH = this._cssMinHeightNumeric;
	    else {
		if (this._cssMinHeightNumeric < this._cssHeightNumeric) 
		    // minH = this._cssMinHeightNumeric + _padYDelta;
		    minH = this._cssMinHeightNumeric;
		else 
		    minH = this._cssHeightNumeric;
	    }
...
*/
	...
	_setOptions: function( options, flags ) {
		...
		that._setOption( key, value, flags );
	...
	_size: function() {
	    ...
	    if ( this._cssMinWidthNumeric > this._cssWidthNumeric ) {
		this._cssWidthNumeric = this._cssMinWidthNumeric;
	    }
		...
		height: "auto",
		width: this._cssWidthNumeric
	    })
		.outerHeight();

	    minContentHeight = Math.max( 0, this._cssMinHeightNumeric - nonContentHeight );
	    maxContentHeight = typeof this._cssMaxHeightNumeric === "number" ?
		Math.max( 0, this._cssMaxHeightNumeric - nonContentHeight ) :
		"none";
		...
		this._resizableComponent( "option", "minHeight", this._minHeight() );
	    }
	},
	...
	_createOverlay: function() {
	    ...
	    //
	    this.overlay = $("<div>")
		.addClass("oj-dialog-overlay oj-dialog-front");
